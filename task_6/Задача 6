...
#define MYDIR "/home/romol99"

char newfiles[256] = "/home/romol99/backup/";
...
void dif(const char * file, const char * name){
	...
	int fd2 = open(newfile, O_WRONLY|O_CREAT|O_TRUNC, S_IRWXU|S_IRGRP|S_IROTH);
        int fd = open("/home/romol99/backup/log.txt", O_WRONLY|O_CREAT|O_TRUNC, S_IRWXU|S_IRGRP|S_IROTH);
        if(fd < 0) {
                exit(1);
        }
	dup2(fd, 1);
	...
	system("rm /home/romol99/backup/log.txt");
	close(fd);
	close(fd2);
	return;
}
...
int iftext(const char * file){
	...
	dup2(fd, 1);
        system(arg);
	close(fd);
        fd = open("/home/romol99/backup/log.txt",O_RDONLY);
        struct stat statbuf;
        fstat(fd, &statbuf);
